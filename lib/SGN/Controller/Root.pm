# TreeImprovementBase Modifications
#   Add a condition to the `auto` function that restricts access to
#   almost all routes if a user is not logged in.
...
sub auto : Private {
    ...
    unless( $c->config->{'disable_login'} ) {
		...
		$insert_handle->execute($sp_person_id);
            ...
            my @allowed_routes = ("/user/login", "/ajax/user/login", "/authenticate/check/token", "/authenticate/caddy");
